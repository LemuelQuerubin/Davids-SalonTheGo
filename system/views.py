...
def walkInNext(request):
    ...
    if request.method == 'POST':
        ...
        create.save()
        request.session['appointment'] = "submitted"
    return render(request, 'appts_walkinnext.html')
...
def clientViewAppointment(request):
    appointments= appointments.object.get(pk=id) #di ko pa alam ano kukunin para maview sariling appointment hehehe
    return render(request, 'clientViewAppointment')
